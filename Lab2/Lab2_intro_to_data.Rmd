...
- The first graph (bins = 0) shows that there are around 27,000 flights on time.
- The second graph (bins = 15) shows that there are around 21,000 flights on time. It also shows that there are almost 3,000 flights that departed earlier than the departure time.
- The third graph (bins = 150) shows that there are more than 30,000 flights on time.
...
Now, we can find the number of flights that meet the criteria using the summary function
```{r summary_sfo}
sfo_feb_flights %>% 
  summarize(n = n())
```
...
Distribution of the arrival delays of these flights
```{r plot-distribution-arrival-delays-sfo}
ggplot(sfo_feb_flights, aes(arr_delay)) +
  geom_histogram()
```

The distribution is skewed, the median is more useful to summarize the data.
...
50% of the flights arrived 11 minutes before scheduled arrival time
...
```
United Air Lines Inc. has the most variable arrival delays 
...
- Mean: It is sensitive as it takes all data values into account but it gives a biased output if outliers/ extreme values exist in the data
- Median: It is not affected by extreme values but it is less sensitive than the mean and only focus on giving out the middle point irrespective of the spread of the data. 
...
As the distance  increases, the average speed also increases at first, but then,  becomes constant.
...
The cutoff point would be approximately 80min because many flights made it on time although they have been delayed 0-80 mins. 
