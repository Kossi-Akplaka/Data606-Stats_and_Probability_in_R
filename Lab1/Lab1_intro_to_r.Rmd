...
```{r}
# Let's plot the numbers of girls baptized over the years
ggplot(data = arbuthnot, aes(x = year, y = girls)) + geom_line()
```
   

Yes, there is an apparent trend in the number of girls baptized over the years. We can see an upward trend from 1660 to the end of the experiment (1710)
...
Throughout the experiment, boys have outnumbers girls every year. The proportion of boys born over time varies up and down.
...
```{r}
# Find the years included in this data set
present$year
# Dimensions and variable names of the data frame
glimpse(present)
```

This data set included the year 1940 to the year 2002
It has 63 rows and 3 columns
The name of the columns is: year, boys, girls

1.  How do these counts compare to Arbuthnot's? Are they of a similar magnitude?

```{r}
# Count of boys anf girls births in present data set
present$boys
present$girls
# Count of boys and girls births in the arbuthnot data set
arbuthnot$boys
arbuthnot$girls
```

No, it is not the same magnitude. We have way more birth's count in the present data set than the arbuthnot data set. 
...
# Plot of the proportion of boys
ggplot(data = present, aes(x = year, y = boy_ratio)) +geom_point()
```

Yes, the observation about boys being born in greater proportion still hold.
...
```{r}
# Let's calculate the total of births
...
We saw the most total births in the U.S. in 1961.
