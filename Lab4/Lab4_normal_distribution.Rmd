...
ggplot(dairy_queen, aes(x = cal_fat)) +
  geom_histogram() +
  ggtitle("Distribution of Dairy Queen")
```

After visualizing both plots, we observe that both plot are centered around 250 calories from fat. Both distribution looks like a bell-shaped but the distribution of McDonald looks less spread than the distribution of Dairy Queen.
...
Yes, it appears that the data is following a normal distribution
...
ggplot(data = NULL, aes(sample = sim_norm)) +
  ...
  stat_qq_line()
```
The plot for normal simulation distribution  still shows points that tend to follow the line.
...
Yes, We can see that the normal probability plot for the calories from fat looks similar to the plots created for the simulated data. This provides evidence that the Dairy queen calories per fat are nearly normal. 
...
The simulated normal distribution almost follow the diagonal line in contrary to the Mc Donald's distribution. This shows that the Mc Donald's distribution *is not* normal distribution.
...
Let's take Arbys as an example

```{r}
arbys <- fastfood %>%
  filter(restaurant == "Arbys")
```
Question 1: "What is the probability that a randomly chosen Arbys product has less than 500 calories?"

- Theoretical normal distribution

```{r}
pnorm(q = 500, mean = mean(arbys$calories), sd = sd(arbys$calories))
```
...
Question 2: "What is the probability that a randomly chosen Arbys product has more than 300 calories per fat?"

- Theoretical normal distribution

```{r}
1 - pnorm(q = 300, mean = mean(arbys$cal_fat), sd = sd(arbys$cal_fat))
```
...
The second question has a much closer agreement between the empirical and the theoretical normal distribution.
...
It seems like Burger King and Taco Bell has the most normal distribution for sodium.
...
Categorization of the sodium made the data appear more discrete, leading to a departure from the smooth diagonal line. 
...
- Let's make a normal probability plot for Taco Bell

```{r}
taco <- fastfood %>%
 filter(restaurant == "Taco Bell")

ggplot(data = taco, aes(sample = total_carb)) +
  geom_line(stat = "qq") +
  stat_qq_line()
```
...
```{r}
ggplot(data = taco, aes(total_carb)) +
