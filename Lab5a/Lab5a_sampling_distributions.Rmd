...
In the sample 1,  we observe that 86% of the population believe that work scientists benefit them while 14% of the population doesn't believe it.
...
No I don't expect the sample proportion to match the sample proportion of another student because it is unlikely that we draw the same sample (unless we both set the seed to be the same). I would expect the proportions to be somewhat different.
...
In sample 1, 14% doesn't believe in scientific work while in sample 2, 24% doesn't believe in it. The sample of size 1000 will provide a more accurate estimate of the population. As the sample size increases, the estimate tends to become more precise and closer to the true population parameter.
...
```{r review-sample_props50}
ggplot(data = sample_props50, aes(x= n)) +
  geom_histogram(binwidth = 2) +
  labs(title = "Sampling distribution of number of elements",
       x = "n count (Doesn't benefit)",
       )
```

The sample distribution follows a normal distribution centered around 10.
...
Each observation represents the sample proportion of "Doesn't benefit" obtained from one of the 5,000 simulations for a specific sample size. As the size increases, the mean gets closer to the true population mean ~0.2, the standard error decreases and the distribution becomes more normal.
...
Using this sample, 87% of people think that work scientists enhance their lives
...
sample_props15 = global_monitor %>% 
  rep_sample_n(15, 2000, replace = TRUE) %>% 
  count(scientist_work) %>% 
  mutate(p_hat = n/sum(n)) %>% 
...
ggplot(data = sample_props15, aes(x = p_hat)) +
  ...
  labs(x = "Benefits",
       title = "Proportion of people who think work scientists benefit them")
```

Based on this sampling distribution, 80% of people think that scientific works enhance their lives
...
set.seed(1515)
sample_props15 = global_monitor %>% 
  rep_sample_n(150, 2000, replace = TRUE) %>% 
  count(scientist_work) %>% 
  mutate(p_hat = n/sum(n)) %>% 
  filter(scientist_work == "Benefits")
...
Based on this distribution, 80% of the population believe that work scientists benefit them. The sampling distribution for sample_props150 looks more normal than the sample_props15 distribution.
...
I will prefer smaller spread because  the sampling distribution are more tightly clustered around the true population parameter and the standard error is smaller
