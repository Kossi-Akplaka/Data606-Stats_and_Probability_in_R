...
- A streak length of 1 means that Kobe made exactly one consecutive basket before missing. In this case, there is one hit (H) followed by one miss (M) in a row.

- A streak length of 0 means that Kobe didn't make any consecutive baskets. In other words, after a hit (H), he immediately followed it with a miss (M), or he missed (M) and then made a basket (H) without any consecutive hits. 
...
ggplot(data = kobe_streak, aes(x = length)) +
  geom_bar()
```
Here, we can see that Kobe's typical streak is length 0 which he has almost 40 times. His longest streak was 4.
...
During my simulation of the unfair coin, 23 came up heads and 77 came up tails.
...
ggplot(sim_streak, aes(x = length)) +
  geom_bar()
```
 We observed that the independent shooter has a typical streak length of 0 (with over 40 streaks). However, the longest streak of the independent shooter is length 5. 
...
If you were to run the simulation of an independent shooter a second time, you would expect its streak distribution to be somewhat similar but not exactly the same as the distribution from the first run.

- Somewhat Similar: In each simulation run, you are essentially generating a sequence of random shots (hits and misses) independently. While the overall distribution characteristics, such as the average streak length and the general shape of the streak distribution, might be similar, the specific streak lengths and patterns are likely to differ due to the randomness involved.

- Not Exactly the Same: It's highly unlikely that the streak distribution would be exactly the same in two different simulation runs. This is because randomness plays a significant role, and even with the same parameters and assumptions, the exact sequence of random outcomes (hits and misses) will differ each time you run the simulation. As a result, the specific streaks observed in one run are unlikely to match those of another run precisely.
...
- Streak Length 0: The independent player had more streaks of length 0 compared to Kobe. This suggests that the independent player had more instances where they alternated between hits and misses without making consecutive baskets, while Kobe may have had longer streaks of consecutive baskets.

- Longest Streak: The independent player had a streak length of 5, while Kobe's longest streak was only 4. This implies that the independent player had a longer sequence of consecutive baskets than Kobe.

Based on these observations, it seems that the streak patterns of the independent player and Kobe do not align with the hot hand model.
