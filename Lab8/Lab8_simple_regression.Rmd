...
This dataset has 1458 rows and 123 columns
...
I can use a scatterplot to plot the relationship between `pf_score` and `pf_expression_control`. The relationship looks linear. It seems like higher the 'pf_expression_control' (the least political pressures and controls on media content), the higher the 'pf_score' (freedom score). I can use a linear model to predict the personal freedom score
...
ggplot(hfi, aes(x = pf_expression_control, y = pf_score)) +
  ...
  ggtitle("pf_expression_control vs. pf_score")
```
...
- Form: The relationship between "pf_expression_control" and "pf_score" appears to be somewhat linear. 

- Direction: The direction of the relationship is positive As the "pf_expression_control" score increases, the "pf_score" tends to increase. 

- Strength: The strength of the relationship is strong because the correlation is higher than 0.7

- Unusual Observations: There appear to be a few outliers in the plot, these outliers could be countries with unique circumstances that deviate from the overall trend.
...
Using plos_ss, the line that minimize the sum of squares has an intercept = 5.21 and the sum of squares = 1118.24
...
A positive slope tells us that the higher the pf_expression_control (i.e. the low the amount of political
    pressure on media content), the higher the hf_score (i.e. the better the human freedom)
...
Yes we can predict the country personal freedom by replacing x with 6.7 in the equation.
...
The predicted personal freedom is 7.909651 with a rating of 6.7. If the actual score is higher than 7.909651, then 7.909651 is an underestimate. If the actual score is lower, then 7.909651 is an overestimate.

However, the point 6.7 doesn't exist in the data so there is no way to know the actual value.
...
There is no apparent pattern, the residuals are randomly scattered around the horizontal line at y = 0. It suggests that the linearity assumption is likely met. The relationship between the two variables is described by the linear model.
...
Yes It does. The distribution looks nearly like  a normal distribution. 
...
Yes, the constant variability condition appear to be met because the residuals are evenly scattered around the horizontal line at y = 0, and there is no discernible funnel-like shape. 
...
I can use a scatterplot to plot the relationship between Economic freedom regulation score `ef_regulation` and Business regulation `ef_regulation_business`.

```{r}
ggplot(hfi, aes(x = ef_regulation_business, y = ef_regulation)) +
  geom_point() +
  ggtitle("ef_regulation_business vs. ef_regulation")

```

The relationship looks linear, we can quantify the strength of the relationship with the correlation coefficient.

```{r }
hfi %>%
  summarise(cor(ef_regulation_business, ef_regulation, use = "complete.obs"))
```

The strength of the relationship is strong because the correlation is higher than 0.7
...
First we can find the $R^2$ of this model.

```{r}
m3 <- lm(ef_regulation ~ ef_regulation_business, data = hfi)
summary(m3)$r.squared # model for `ef_regulation_business` and `ef_regulation`
summary(m1)$r.squared # model for `pf_expression_control` and `pf_score`
```

The first model  pf_expression_control vs pf_score perform better than the other model ef_regulation_business vs ef_regulation. 
...
I was interested in  the relationship between Economic freedom regulation score `ef_regulation` and Business regulations - Extra payments/bribes/favoritism `ef_regulation_business_bribes`.

First, we can plot the data.

```{r}
ggplot(hfi, aes(x = ef_regulation_business_bribes, y = ef_regulation)) +
  geom_point() +
  ggtitle("ef_regulation_business_bribes vs. ef_regulation")

```

Now let's find the $R^2$ of the model

```{r}
m4 <- lm(ef_regulation ~ ef_regulation_business_bribes, data = hfi)
summary(m4)$r.squared 
```

This model performs very poorly. I thought the "Business regulations - Extra payments/bribes/favoritism' would be a great predictor of the "Economic freedom regulation".
